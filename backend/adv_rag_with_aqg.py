...
PROMPT_FILES = {
    'Resume Reviewer': {
        ...
        'advanced': '../prompts/Resume_reviewer/adv_rag_with_agumented_query_generated.txt'
    },
    'Contract Legal Advisor': {
        'base': '../prompts/legal_contract_advisor/base_prompt.txt',
        'advanced': '../prompts/legal_contract_advisor/adv_rag_with_agumented_query_generated.txt'
    }
}
...
def get_prompts():
    available_prompts = {}
    for task, prompt_paths in PROMPT_FILES.items():
        available_prompts[task] = list(prompt_paths.keys())
...
# Custom RAG pipeline to handle questions with dynamic prompt selection and joint query
@app.route('/ask-pipeline2', methods=['POST'])
def ask_question():
    ...
    try:
        ...
        # Step 1: Augment the query with a hypothetical answer using the selected advanced prompt
        if prompt_complexity == 'advanced':
            augmented_query = augment_query_generated(question, advanced_prompt)
        else:
            augmented_query = augment_query_generated(question, base_prompt)

        joint_query = f"{question} {augmented_query}"

        # Step 2: Retrieve relevant documents using the joint query
        ...
        # Step 3: Use the RAG function to generate an answer based on the retrieved documents
        # Always use the base prompt for generating the final answer
        answer = rag(query=question, retrieved_documents=retrieved_documents, prompt=base_prompt)
...
# Function to generate a hypothetical answer and augment the query with dynamic prompt selection
def augment_query_generated(query, selected_prompt, model="gpt-4o-mini"):
    messages = [
        {
            "role": "system",
...
def rag(query, retrieved_documents, prompt, model="gpt-4o-mini"):
